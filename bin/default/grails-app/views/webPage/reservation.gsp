	...
	$(document).ready(function() {
            $('.row button').click(function(){
				...
				if(status == "btn btn-common"){
					...
					var URL="${createLink(controller:'webPage',action:'send')}"

					$.ajax({
						url: URL,
						type: "POST",
						datatype: "html",
						data:{date:input_date,time:input_time,person:person,name:name,email:email,phone:phone},
						success:function(data)
						{
							$('#msgSubmit').html(data);
							$('#input_date').val("");$('#person').val("");$('#email').val("");
							$('#input_time').val("");$('#name').val("");$('#phone').val("");
						}
					}) 
				}
            });
